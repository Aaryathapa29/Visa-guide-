...
export const Route = createFileRoute("/countries")({
  ...
  head: () => ({
    meta: [
      { title: "Explore — Student Visas & Consultancies | VisaGuide" },
      {
        name: "description",
        content: "Explore student-visa destinations and partner consultancies for USA, Canada and Australia.",
      },
    ],
  }),
});
...
function CountriesPage() {
  ...
  return (
    <div>
      {/* Hero — minimal navy banner, at least 50vh */}
      <section
        className="relative flex min-h-[50vh] items-center overflow-hidden border-b border-white/10 bg-primary text-primary-foreground"
      ...
      >
        ...
        <div className="relative w-full px-6 py-16 md:px-12 lg:px-16">
        ...
        {COUNTRIES.map((c) => (
          ...
          >
            ...
            <div className="flex items-start justify-between">
              <span className="grid h-14 w-14 place-items-center rounded-full border-2 border-border bg-secondary text-3xl transition-colors group-hover:border-gold">{c.flag}</span>
              <span className="rounded-sm bg-primary px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-primary-foreground">
                {c.visaType.split(" ")[0]}
              </span>
            </div>
            <h2 className="mt-5 font-serif text-2xl text-primary">{c.name}</h2>
