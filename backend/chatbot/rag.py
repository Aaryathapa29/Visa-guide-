...
Lazy loading (why startup stays fast):
  The heavy dependencies (torch/transformers via sentence-transformers, and
  chromadb) are imported INSIDE functions, and the embedding model + ChromaDB
  collection are created on first use, never at import time. This keeps Django's
  `runserver` and every `manage.py` command fast; the ~model-load cost is paid
  once, on the first chat/upload request.
"""
...
def get_embedder():
    """Load the sentence-transformers model once, on first use (not at import)."""
    global _embedder
    if _embedder is None:
        # Imported here so torch/transformers only load when embeddings are needed.
        from sentence_transformers import SentenceTransformer
        logger.info("Loading embedding model (%s)... first call only", EMBEDDING_MODEL)
...
def embed(texts: list[str]) -> list[list[float]]:
    ...
    vectors = get_embedder().encode(texts, convert_to_numpy=True)
...
def ingest_document(path: Path, country: str, collection, source_name: str | None = None) -> int:
    ...
    logger.info("Ingesting: %s (%s)", name, country)
...
def detect_countries(message: str) -> list[str]:
    """Return every supported country the message mentions, in registry order."""
    msg = message.lower()
    found = []
    for country, keywords in COUNTRY_KEYWORDS.items():
...
def detect_country(message: str) -> str | None:
    """Detect the single most relevant country (first match), or None."""
    countries = detect_countries(message)
...
# ── RAGEngine: the main class used by the views ───────────────────────────────
class RAGEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("ChromaDB looks incompatible/corrupt (%s: %s). Rebuilding...",
                           type(e).__name__, e)
            reset_chroma_store()
            self._open_and_sync()
        count = self.collection.count()
        logger.info("RAG ready with %d chunks", count)
    ...
    def retrieve(self, query: str) -> list[dict]:
        ...
        countries = detect_countries(query)
        ...
        # Filter to a single country only when exactly one is mentioned. For zero
        # or several (for example "compare USA and Canada") search across all docs
        # so no country is silently dropped.
        where = {"country": {"$eq": countries[0]}} if len(countries) == 1 else None
        results = _query(where)
        if where and not results:
            results = _query(None)
        return [r for r in results if r["score"] >= RELEVANCE_THRESHOLD]

    def build_context(self, query: str, retrieval_query: str | None = None) -> tuple[str, list[dict]]:
        """
        Build numbered context for the LLM and the matching source list.
        Greetings / small talk skip retrieval (no context, no sources).

        `query` is the user's original message (used for small-talk detection);
        `retrieval_query`, when given, is the history-aware, standalone query used
        for the actual vector search (see services.condense_query).
        """
        if is_smalltalk(query):
            return "", []

        results = self.retrieve(retrieval_query or query)
        if not results:
