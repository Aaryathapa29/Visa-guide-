...
import { Bot, CalendarDays, FileSearch, ArrowRight, Sparkles } from "lucide-react";
    ...
    <div className="min-h-screen" style={{ background: "#f0f4f8" }}>
      <main className="aspirant-shell">
        ...
        <section className="border-y border-slate-200 bg-white"><div className="mx-auto max-w-7xl px-6 py-14 md:px-12 lg:px-16"><p className="text-[11px] font-semibold uppercase tracking-[.25em] text-[#f97316]">AI Toolkit</p><h2 className="aspirant-serif mt-2 text-3xl text-[#0a1f44]">Make every step clearer.</h2><div className="mt-8 grid gap-5 md:grid-cols-3">{[
          { label: "Document Analyser", sub: "Review your cover letter before submission.", icon: <FileSearch className="h-6 w-6" />, onClick: () => navigate("/document-analyzer") },
          { label: "Visa AI Assistant", sub: "Ask visa questions anytime, in plain language.", icon: <Bot className="h-6 w-6" />, onClick: () => navigate("/chatbot") },
      ...
      <button onClick={() => navigate("/chatbot")} className="fixed bottom-6 right-6 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-[#f97316] text-white shadow-lg transition-all hover:scale-105 active:scale-95" aria-label="Open Visa Guide Assistant">
